...
struct GraphicRenderPass {
    ...
    init(view: MTKView, physicPass: PhysicRenderPass, quad: QuadModel, camera: OrthographicCamera) {
        ...
        self.informationBuffer = physicPass.informationBuffer
    ...
    func draw(commandBuffer: MTLCommandBuffer) {
        ...
        var simParams = self.simParams
        renderEncoder.setFragmentBytes(&simParams, length: MemoryLayout<SimParams>.stride, index: SimParamsBuffer.index)
        renderEncoder.setFragmentBuffer(informationBuffer, offset: 0, index: InformationBuffer.index)
